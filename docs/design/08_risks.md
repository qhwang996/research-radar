...
### D19: 双轨处理 > 拍平排序 (2026-03-16)
学术来源和工业来源分轨处理。论文告诉你「学术界在做什么」，博客告诉你「工业界在痛什么」，拍平排序会丧失这种结构性差异。详见 09_intelligence_layer.md v2。

### D20: 统计空白检测 > 纯 LLM 推理链 (2026-03-16)
方向发现的核心用统计方法（词频交叉比对），LLM 只做最后验证。Haiku 推理能力有限，四步 LLM 链误差会累积；统计方法对个体错误有天然容错。

### D21: Profile 放宽为领域过滤 (2026-03-16)
清空 preferred_topics，只保留 domain_scope + avoided_topics 做宽泛过滤。目标是让系统能发现用户「不知道自己会感兴趣的」方向。全量 re-score (v4)，成本 ~$0.50。

### D22: 需求信号存 summary_l2 (2026-03-16)
博客 demand signal JSON 存入 Artifact.summary_l2，与论文 L2 JSON 共用字段。通过 signal_type 字段区分格式。避免 schema 迁移。

### D23: 轨道分离是逻辑分离 (2026-03-16)
两条轨道共用 artifacts 表，分离在 pipeline 路由和评分权重中实现。简单，支持跨轨查询。
