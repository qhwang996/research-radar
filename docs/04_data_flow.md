...
### 3.1 Normalization (P2.1)
纯数据转换，不调用LLM：
```
1. 扫描未处理的raw data（通过RawFetch记录追踪）
2. 提取元数据（作者、时间、来源等）
3. 生成canonical_id（UUID5，基于内容hash）
4. 去重（基于canonical_id upsert）
5. 保存为结构化artifact
6. 更新RawFetch处理状态
```
...
- `summary_l1/l2/l3`: LLM生成的多层次摘要（由Enrichment填充）
- `recency_score/authority_score/relevance_score/final_score`: 评分（由Scoring填充）
- `tags`: 标签列表
